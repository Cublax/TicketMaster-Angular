...
export class ApiService {
  ...
  getDiscoveryEvents(): Observable<GridEvent[]> {
  ...
  mapToGridFormat(data: TicketMasterData): GridEvent[] {
    return data._embedded.events.map(event => {
        return new GridEvent(
          ...
          event.priceRanges ? (event.priceRanges[0].min.toString() +" " + event.priceRanges[0].currency) : "0",
          event.url
        )
    }
    )
  ...
  searchEvents(term: String): Observable<GridEvent[]> {
